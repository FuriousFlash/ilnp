...
//@kbteja.. here the IP address is changed without the use of DAD.
//now we trigger the handover process in ILNP by using 2 methods..
//method1 :- we subscribe the ILCC Module to the notification Board and then when we observe a 
//change in the prefix of an interface, we trigger the handoff procedures.
}
...
// @kbteja.. 
// choose preferred address on a particular interface should always return an ILNP address.
// so the Code has to be modified in such a way that an ILNP address with highest expiry
// time is returned.
