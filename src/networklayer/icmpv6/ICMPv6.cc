...
void ICMPv6::processICMPv6Message(ICMPv6Message *icmpv6msg)
{
    ...
    //@kbteja.. code to process ILNPv6 LU/LU-ACK messages.
    else if (dynamic_cast<ILNPv6LUMsg *>(icmpv6msg))
    {
        EV << "ILNPv6 LU msg is received" << endl;
        //processILNPLUMsg((ILNPv6LUMsg*)icmpv6msg);
    }
    else if (dynamic_cast<ILNPv6LUAckMsg *>(icmpv6msg))
    { 
        // @kbteja
        // code here.. call the appropriate function in ILCache module.
    }
    else
...
// @kbteja.. code for sending LU, LU-ACK Messages and processing the received LU/LU-ACK
// messages.

void ICMPv6::sendILNPLUMsg(const IPv6Address& destILNPAddr, 
    InterfaceEntry* ie, const int buSeq, const int lifeTime)
{
    ILNPv6LUMsg* luMsg = new ILNPv6LUMsg();
    luMsg->setCode(151);
    luMsg->setSeqNo(buSeq);
    luMsg->setLifeTime(lifeTime);
    IPv6ControlInfo *ctrlInfo = new IPv6ControlInfo();
    ctrlInfo->setDestAddr(destILNPAddr);
    ctrlInfo->setInterfaceId(ie->getInterfaceId());
    ctrlInfo->setProtocol(IP_PROT_IPv6_ICMP);
    luMsg->setControlInfo(ctrlInfo);
    sendToIP(luMsg);
}

void ICMPv6::sendILNPLUAckMsg(const IPv6Address& destILNPAddr, 
    InterfaceEntry* ie, const int buSeq, const int lifeTime)
{
    ILNPv6LUAckMsg* luAckMsg = new ILNPv6LUAckMsg();
    luAckMsg->setCode(152);
    luAckMsg->setSeqNo(buSeq);
    luAckMsg->setLifeTime(lifeTime);
    IPv6ControlInfo *ctrlInfo = new IPv6ControlInfo();
    ctrlInfo->setDestAddr(destILNPAddr);
    //int interfaceId = ie->getInterfaceId;
    ctrlInfo->setInterfaceId(ie->getInterfaceId());
    ctrlInfo->setProtocol(IP_PROT_IPv6_ICMP);
    luAckMsg->setControlInfo(ctrlInfo);
    sendToIP(luAckMsg);
}

/*void ICMPv6::processILNPLUMsg(ILNPv6LUMsg* luMsg)
{
    IPv6ControlInfo *ctrl = check_and_cast<IPv6ControlInfo*>(luMsg->removeControlInfo());
    int seqNo = luMsg->getSeqNo();
    int lifeTime = luMsg->getLifeTime();
    IPv6Address srcILNPAddr = ctrl->getSrcAddress();
    ilnp->processReceivedLU(srcILNPAddr,buSeq,lifeTime);
} */

/*
void ICMPv6::processILNPLUAckMsg(ILNPv6LUAckMsg* luAckMsg)
{
    IPv6ControlInfo *ctrl = check_and_cast<IPv6ControlInfo*>(luMsg->removeControlInfo());
    int seqNo = luAckMsg->getSeqNo;
    int lifeTime = luAckMsg->getLifeTime;
    IPv6Address srcILNPAddr = ctrl->getSrcAddress();
    ilnp->processReceivedLUAck(srcILNPAddr,buSeq,lifeTime);
}*/
